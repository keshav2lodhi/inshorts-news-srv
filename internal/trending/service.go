...
func (s *TrendingService) StartEventSimulation(articles map[string]news.Article) {
	if len(articles) == 0 {
	...
	for {
		select {
		case <-eventTicker.C:
			a := articleList[rand.Intn(len(articleList))] // Pick random article

			eventType := EventView // Random event type
			if rand.Float64() < 0.3 {
				eventType = EventClick
			}

			latOffset := (rand.Float64() - 0.5) * 0.05 // Slight geo variation near article
